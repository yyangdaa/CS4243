...
class CTCCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        string_acc = correct_strings / total_strings if total_strings > 0 else 0
        char_acc = correct_chars / total_chars if total_chars > 0 else 0

        # Log string accuracy so that it can be monitored
        logs['string_acc'] = string_acc

        print(f"\n✅ Epoch {epoch+1}: String Accuracy: {string_acc:.2%} | 🔠 Character Accuracy: {char_acc:.2%}")

# --- Custom Early Stopping Callback that only activates after 20 epochs ---
class DelayedEarlyStopping(tf.keras.callbacks.Callback):
    def __init__(self, monitor='string_acc', patience=5, start_epoch=20, mode='max'):
        super().__init__()
        self.monitor = monitor
        self.patience = patience
        self.start_epoch = start_epoch
        self.mode = mode
        self.best = -np.inf if mode == 'max' else np.inf  # use np.inf instead of np.Inf
        self.wait = 0

    def on_epoch_end(self, epoch, logs=None):
        logs = logs or {}
        if epoch < self.start_epoch:
            return  # Skip early epochs
        ...
        if self.mode == 'max':
            if current > self.best:
                ...
                if self.wait >= self.patience:
                    print(f"\n🛑 Early stopping triggered at epoch {epoch+1} with {self.monitor}: {current:.2%}")
            ...
            if current < self.best:
                ...
                self.wait += 1
                if self.wait >= self.patience:
                    print(f"\n🛑 Early stopping triggered at epoch {epoch+1} with {self.monitor}: {current}")
                    self.model.stop_training = True
...
model.fit(
    ...
    callbacks=[
        ...
        DelayedEarlyStopping(monitor='string_acc', patience=5, start_epoch=20, mode='max')
    ]
)
